"""Analyse BH2O-36 hydrolysis reaction barrier benchmark."""
...
CALC_PATH = CALCS_ROOT / "single_point" / "bh2o-36" / "outputs"
OUT_PATH = APP_ROOT / "data" / "single_point" / "bh2o-36"
...
def barrier_heights() -> dict[str, list]:
    ...
    for model_name in MODELS:
        for label in labels():
            ...
            results[model_name].append(atoms_ts.info['pred_energy'] - atoms_rct.info['pred_energy'])
            results[model_name].append(atoms_ts.info['pred_energy'] - atoms_pro.info['pred_energy'])
            
            if not ref_stored:
                results['ref'].append(atoms_ts.info['ref_energy'] - atoms_rct.info['ref_energy'])
                results['ref'].append(atoms_ts.info['ref_energy'] - atoms_pro.info['ref_energy'])
