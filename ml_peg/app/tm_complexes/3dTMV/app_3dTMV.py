...
DOCS_URL = (
    "https://ddmms.github.io/ml-peg/user_guide/benchmarks/tm_complexes.html#3dtmv"
)
...
class TMVApp(BaseApp):
    ...
    def register_callbacks(self) -> None:
        ...
        scatter = read_plot(DATA_PATH / "figure_ip.json", id=f"{BENCHMARK_NAME}-figure")
        ...
        try:
            with open(DATA_PATH / "ids.json") as fh:
                ids = json.load(fh)
            # Assets dir is parent of DATA_PATH
            structs = [
                f"assets/tm_complexes/3dTMV/{MODELS[0]}/{cid}.xyz" for cid in ids
            ]
        except Exception:
            pass
...
def get_app() -> TMVApp:
    ...
    return TMVApp(
        name=BENCHMARK_NAME,
        description=("Vertical ionization potentials (kcal/mol) for 3d TM complexes."),
        docs_url=DOCS_URL,
