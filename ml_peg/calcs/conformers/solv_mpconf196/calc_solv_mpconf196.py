...
class solvMPCONF196_Benchmark(zntrack.Node):
    ...
    def get_ref_energies(self):
        df = pd.read_excel(DATA_PATH / 'Energies_CCSD(T).xlsx', sheet_name='Total PNO-CCS(T) Energies solvM', header=1)
        self.ref_energies = {}
        ...
        for molecule in MOLECULES:
            ...
            for label, E_ref in self.ref_energies.items():
                ...
                if molecule != group_label:
                    continue
                atoms = self.get_atoms(DATA_PATH / 'solvMPCONF196_geometries/solvMPCONF196' / f'{label}/struc.xyz')
