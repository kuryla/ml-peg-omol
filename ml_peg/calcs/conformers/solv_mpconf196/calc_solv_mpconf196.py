...
class solvMPCONF196_Benchmark(zntrack.Node):
    
    model: NodeWithCalculator = zntrack.deps()
    model_name: str = zntrack.params()

    def get_ref_energies(self):
        df = pd.read_csv(DATA_PATH / 'Energies_CCSD(T).xlsx', sheet_name='Total PNO-CCS(T) Energies solvM', header=3)
        self.ref_energies = {}
        for row in df.iterrows():
            label = row[1][0]
            E_ref = float(row[1][4]) * KCAL_TO_EV
    ...
    def run(self):
        ...
        for molecule in MOLECULES:
            ...
            for label, E_ref in self.ref_energies.items():
                ...
                atoms = read(DATA_PATH / 'solvMPCONF196_geometries' / label / 'struc.xyz')
                atoms.calc = calc
...
def build_project(repro: bool = False) -> None:
    ...
    for model_name, model in MODELS.items():
        with project.group(model_name):
            benchmark = solvMPCONF196_Benchmark(
                model=model,
